...
const store = new Vuex.Store({
    ...
    actions: {
        ...
        RENDER_MATHJAX: ctx => Vue.nextTick(_ => MathJax.Hub.Queue(["Typeset", MathJax.Hub])),
        CREATE_NOTE: ({
            state,
            commit,
            dispatch
...
const app = {
    ...
    methods: {
        ...
        updateSelectedTag: function(e) {
            ...
            let cleanTags = rawTags.slice(0, rawTags.length - 1).map(tag => tag.replace(/^\s+/g, '').replace(/\s+$/g, '')).filter(tag => tag.length > 0);
