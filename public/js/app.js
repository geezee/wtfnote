...
const formatters = [function(body) {
    return loadScript('showdown', 'js/showdown.min.js').then(() => {
...
const SelectedNote = {
    ...
    mutations: {
        ...
        DESELECT_NOTE: state => state.selectedNote = makeEmptyNote(),
        RENDER_SELECTED_NOTE: state => {
            // chain all the formatters so one feeds its body to the other
            formatters.reduce((promise, formatter) => promise.then(body => formatter(body, state.selectedNote)), Promise.resolve(state.selectedNote.body))
                // then set the html property
                .then(html => {
                    Vue.set(state.selectedNote, 'html', html);
                });
        },
        REMOVE_ATTACHMENT: (state, index) => Vue.delete(this.selectedNote.attachments, index),
    },
    getters: {
        ...
        getSelectionVersion: state => (state.selectedNote.id != state.emptyNoteTemplate.id && state.selectedNote.versions.length > 0) ? state.selectedNote.versions[state.versionNumber] : {
            createdAt: '',
            body: ''
        },
    },
    actions: {
        ...
        DELETE_SELECTED_NOTE: ({
            ...
            Vue.http.get(`./api/note/${selectedNoteId}/delete`).then(request => {
        ...
        SAVE: ({
            ...
            return new Promise((resolve, reject) => {
                state.timer = setTimeout(
                    () => dispatch('FLUSH_AUTOSAVE_QUEUE').then(resolve, reject), state.delay);
            });
        },
        FLUSH_AUTOSAVE_QUEUE: ({
            state,
            getters,
            commit
        }) => {
...
function _note_satisfies(searchQuery) {
    ...
    const ors = searchQuery.split(';').filter(expr => expr.trim().length > 0).map(expr => {
...
const Search = {
    ...
    mutations: {
        ...
        TOGGLE_TAGS_VISIBILITY: state => state.tagsVisible = !state.tagsVisible,
    }
}
const Attachments = {
    state: {
...
const store = new Vuex.Store({
    ...
    actions: {
        ...
        SELECT_FIRST_NOTE: ({
            ...
            commit,
            dispatch
        }) => state.notes.length > 0 ? dispatch('SELECT_NOTE', state.notes[0]) : commit('DESELECT_NOTE'),
        CREATE_NOTE: ({
            state,
            commit,
            dispatch
    ...
    mutations: {
        SORT_NOTES: state => state.notes.sort(function(a, b) {
            ...
            else if (a.versions.length == 0) return -1;
            else if (b.versions.length == 0) return 1;
...
const app = {
    ...
    methods: {
        ...
        updateSelectedTag: function(e) {
            ...
            let cleanTags = rawTags.slice(0, rawTags.length - 1).filter(tag => tag.length > 0);
